...
本プロジェクトは [Claude Code](https://claude.ai/code) を用いた AI 駆動開発で構築した。アプリケーションの実装だけでなく、AI を適切に導くハーネス設計そのものをエンジニアリングの対象とした。

### ハーネス設計: Planner / Generator / Evaluator

[Anthropic のハーネス設計原則](https://www.anthropic.com/engineering/harness-design-long-running-apps) を参考に、各スプリントを Claude Code の `Agent` ツールで起動する3つの専門エージェントで分担した。
