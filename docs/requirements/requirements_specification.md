...
| 層 | フレームワーク | 対象 |
|----|-------------|------|
| Backend / Router | pytest + httpx `AsyncClient` | エンドポイントの入出力・ステータスコード |
| Backend / Service | pytest | ビジネスロジック・バリデーション |
| Backend / Repository | pytest | DB クエリの正確性（本番と同じ PostgreSQL をテスト用コンテナとして使用） |
| Backend / Core | pytest | 設定管理・ロギング設定 |
| Frontend / Component | Vitest + React Testing Library | レンダリング・ユーザー操作 |
| Frontend / Hooks | Vitest | TanStack Query フックの状態管理 |
| Frontend / Lib | Vitest | API クライアント・ユーティリティ関数 |
